...
interface Props { routes: ScoredRoute[]; activeIdx: number; onSelect: (idx: number) => void; loading: boolean; }
...
function ShadeBar({ score }: { score: number }) {
  ...
  return (
    <div className="flex items-center gap-2 mt-1">
      <div className="flex-1 bg-gray-200 rounded-full h-2"><div className={`${color} h-2 rounded-full transition-all`} style={{ width: `${pct}%` }} /></div>
      <span className="text-xs font-semibold w-10 text-right">{pct}% צל</span>
    </div>
  );
}

const ROUTE_NAMES = ['מהיר', 'מוצל', 'מאוזן'];
const ROUTE_LABELS = ['⚡', '🌳', '⚖️'];

export default function RoutePanel({ routes, activeIdx, onSelect, loading }: Props) {
  if (loading) return <div className="flex flex-col gap-2 p-4">{[0,1,2].map(i => <div key={i} className="h-20 bg-gray-100 rounded-xl animate-pulse" />)}</div>;
  if (!routes.length) return <div className="p-4 text-gray-500 text-sm text-center">בחר נקודות מוצא ויעד על המפה</div>;
  ...
  return (
    <div className="flex flex-col gap-2 p-3" dir="rtl">
      ...
      {routes.map((route, idx) => {
        ...
        return (
          <button key={idx} onClick={() => onSelect(idx)} className={`w-full text-right rounded-xl p-3 border-2 transition-all ${isActive ? 'border-blue-500 bg-blue-50 shadow-md' : 'border-gray-200 bg-white hover:border-gray-300 hover:shadow-sm'}`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                {isShadiest && <span className="text-xs bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-full font-medium">הכי מוצל</span>}
              </div>
              <div className="text-xs text-gray-500 flex gap-3"><span>{formatDistance(route.distance)}</span><span>{formatDuration(route.duration)}</span></div>
            </div>
            <ShadeBar score={route.shadeScore} />
            <p className="text-xs text-gray-400 mt-1">{shadePct >= 70 ? 'רוב המסלול בצל' : shadePct >= 35 ? 'חלקית מוצל' : 'חשיפה גבוהה לשמש'}</p>
          </button>
        );
      })}
